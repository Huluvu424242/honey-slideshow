...
const Menu = class {
    constructor(hostRef) {
        ...
        this.lastOnEnd = 0;
        this.blocker = index$1.GESTURE_CONTROLLER.createBlocker({ disableScroll: true });
    ...
    sideChanged() {
        this.isEndSide = helpers$1.isEndSide(this.side);
    }
    swipeGestureChanged() {
        this.updateState();
    }
    async connectedCallback() {
        if (this.type === undefined) {
            this.type = core.config.get('menuType', 'overlay');
        }
...
`);
        ...
        index$4.menuController._register(this);
    ...
    disconnectedCallback() {
        ...
        index$4.menuController._unregister(this);
        if (this.animation) {
    ...
    setOpen(shouldOpen, animated = true) {
        return index$4.menuController._setOpen(this, shouldOpen, animated);
    }
    ...
    async loadAnimation() {
        ...
        this.animation = await index$4.menuController._createAnimation(this.type, this);
        if (!core.config.getBoolean('animated', true)) {
    ...
    canStart(detail) {
        ...
        else if (index$4.menuController._getOpenSync()) {
            return false;
        }
    ...
    onStart() {
        if (!this.isAnimating || !this.animation) {
            helpers$1.assert(false, 'isAnimating has to be true');
            return;
        }
    ...
    onEnd(detail) {
         ...
         */
        newStepValue += cubicBezier.getTimeGivenProgression([0, 0], [0.4, 0], [0.6, 1], [1, 1], helpers$1.clamp(0, adjustedStepValue, 0.9999))[0] || 0;
    ...
    beforeAnimation(shouldOpen) {
        helpers$1.assert(!this.isAnimating, '_before() should not be called while animating');
    ...
    afterAnimation(isOpen) {
        helpers$1.assert(this.isAnimating, '_before() should be called while animating');
    ...
    updateState() {
        ...
        if (!this.disabled) {
            index$4.menuController._setActiveMenu(this);
        }
        helpers$1.assert(!this.isAnimating, 'can not be animating');
    }
    forceClosing() {
        helpers$1.assert(this._isOpen, 'menu cannot be closed');
        this.isAnimating = true;
    ...
    render() {
        ...
        return (core.h(core.Host, { role: "navigation", class: {
            ...
            } }, core.h("div", { class: "menu-inner", part: "container", ref: el => this.menuInnerEl = el }, core.h("slot", null)), core.h("ion-backdrop", { ref: el => this.backdropEl = el, class: "menu-backdrop", tappable: false, stopPropagation: false, part: "backdrop" })));
    }
