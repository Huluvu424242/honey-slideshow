...
const Range = class {
    constructor(hostRef) {
        ...
        this.clampBounds = (value) => {
            return helpers$1.clamp(this.min, value, this.max);
        };
        ...
        this.handleKeyboard = (knob, isIncrease) => {
            ...
            if (knob === 'A') {
                this.ratioA = helpers$1.clamp(0, this.ratioA + step, 1);
            }
            else {
                this.ratioB = helpers$1.clamp(0, this.ratioB + step, 1);
            }
            this.updateValue();
        };
        this.onBlur = () => {
    ...
    onStart(detail) {
        ...
        let ratio = helpers$1.clamp(0, (currentX - rect.left) / rect.width, 1);
        if (document.dir === 'rtl') {
            ratio = 1 - ratio;
        }
        this.pressedKnob =
    ...
    render() {
        ...
        helpers$1.renderHiddenInput(true, el, this.name, JSON.stringify(this.getValue()), disabled);
        return (core.h(core.Host, { onFocusin: this.onFocus, onFocusout: this.onBlur, class: Object.assign(Object.assign({}, theme.createColorClasses(this.color)), { [mode]: true, 'in-item': theme.hostContext('ion-item', el), 'range-disabled': disabled, 'range-pressed': pressedKnob !== undefined, 'range-has-pin': pin }) }, core.h("slot", { name: "start" }), core.h("div", { class: "range-slider", ref: rangeEl => this.rangeSlider = rangeEl }, ticks.map(tick => (core.h("div", { style: tickStyle(tick), role: "presentation", class: {
                'range-tick': true,
                'range-tick-active': tick.active
            }, part: tick.active ? 'tick-active' : 'tick' }))), core.h("div", { class: "range-bar", role: "presentation", part: "bar" }), core.h("div", { class: "range-bar range-bar-active", role: "presentation", style: barStyle, part: "bar-active" }), renderKnob(isRTL, {
            knob: 'A',
...
const renderKnob = (isRTL, { knob, value, ratio, min, max, disabled, pressed, pin, handleKeyboard }) => {
    ...
    return (core.h("div", { onKeyDown: (ev) => {
            const key = ev.key;
        ...
        }, style: knobStyle(), role: "slider", tabindex: disabled ? -1 : 0, "aria-valuemin": min, "aria-valuemax": max, "aria-disabled": disabled ? 'true' : null, "aria-valuenow": value }, pin && core.h("div", { class: "range-pin", role: "presentation", part: "pin" }, Math.round(value)), core.h("div", { class: "range-knob", role: "presentation", part: "knob" })));
};
const ratioToValue = (ratio, min, max, step) => {
    ...
    return helpers$1.clamp(min, value, max);
};
const valueToRatio = (value, min, max) => {
    return helpers$1.clamp(0, (value - min) / (max - min), 1);
};
