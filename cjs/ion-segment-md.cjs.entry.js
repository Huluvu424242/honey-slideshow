...
const Segment = class {
     ...
     */
    addRipple(detail) {
        const useRippleEffect = core.config.getBoolean('animated', true) && core.config.getBoolean('rippleEffect', true);
        if (!useRippleEffect) {
            return;
        }
        const buttons = this.getButtons();
        ...
        const { x, y } = helpers$1.pointerCoord(detail.event);
