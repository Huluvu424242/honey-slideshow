...
const Spinner = class {
    ...
    getName() {
        const spinnerName = this.name || core.config.get('spinner');
    ...
    render() {
        const self = this;
        const mode = core.getIonMode(self);
        ...
        return (core.h(core.Host, { class: Object.assign(Object.assign({}, theme.createColorClasses(self.color)), { [mode]: true, [`spinner-${spinnerName}`]: true, 'spinner-paused': !!self.paused || core.config.getBoolean('_testing') }), role: "progressbar", style: spinner.elmDuration ? { animationDuration: duration + 'ms' } : {} }, svgs));
    }
...
const buildCircle = (spinner, duration, index, total) => {
    ...
    return (core.h("svg", { viewBox: data.viewBox || '0 0 64 64', style: data.style }, core.h("circle", { transform: data.transform || 'translate(32,32)', cx: data.cx, cy: data.cy, r: data.r, style: spinner.elmDuration ? { animationDuration: duration + 'ms' } : {} })));
};
const buildLine = (spinner, duration, index, total) => {
    const data = spinner.fn(duration, index, total);
    data.style['animation-duration'] = duration + 'ms';
    return (core.h("svg", { viewBox: data.viewBox || '0 0 64 64', style: data.style }, core.h("line", { transform: "translate(32,32)", y1: data.y1, y2: data.y2 })));
};
